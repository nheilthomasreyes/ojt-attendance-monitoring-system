...
// ── DELETE /api/students/:id — admin soft-deletes a student ──
// Sets is_active = 0 so attendance history is preserved.
// Student can no longer log in after this.
router.delete('/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    await db.query(`DELETE FROM attendance_logs WHERE student_id = ?`, [id]);
...
router.put('/:id/password', async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      `SELECT id, password FROM students WHERE id = ? LIMIT 1`,
      [id]
    );
